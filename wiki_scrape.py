...
def get_wikipedia_links(page_url: str):
    ...
    for a in content_div.find_all("a", href=True):
        ...
        # Skip namespaces that aren't normal articles like 'help', 'file', 'category', 'wikipedia', etc.
        if ":" in href:
            continue
...
def crawl_wikipedia(
    ...
    """
    Crawl Wikipedia starting from start_url and write edges to CSV:

      source_page, linked_title, linked_url

    Modes:
      - mode="max_pages": stop after visiting `max_pages` source pages.
      - mode="infinite": run until the queue is empty OR user stops with Ctrl+C.

    Notes:
      - "visited" tracks SOURCE pages already crawled.
    """
